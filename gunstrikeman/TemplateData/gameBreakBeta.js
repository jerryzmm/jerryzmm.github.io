...
function showNextAd()
{
    ...
    adBreak({
        ...
        adBreakDone: (placementInfo) => {
            ...
            if(placementInfo.breakStatus == "frequencyCapped"){noInterstialAdsAvailable()};
            if(placementInfo.breakStatus == "other"){noInterstialAdsAvailable()};
            if(placementInfo.breakStatus == "viewed"){interstialAdsViewed()};
        },
    });
}
...
function rewardedAdStart(){
    console.log("cancelReward")
    myGameInstance.SendMessage('Y8AdHandler', 'RewardedAdStart');
}

function noRewardAdsAvailable()
{
    console.log("noRewardAdsAvailable")
    myGameInstance.SendMessage('Y8AdHandler', 'NoRewardedAdsTryLater');
}

function cancelReward()
{
    console.log("cancelReward")
    // myGameInstance.SendMessage('Y8AdHandler', 'resumeGameRewarded');
    myGameInstance.SendMessage('Y8AdHandler', 'NoRewardedAdsTryLater');
}
...
function gainReward()
{
    ...
    // myGameInstance.SendMessage('Y8AdHandler', 'resumeGameRewarded');
    myGameInstance.SendMessage('Y8AdHandler', 'RewardAdsCompleted');
}

function passBeforeAdData() 
{
    myGameInstance.SendMessage('Y8AdHandler', 'PauseGame');
}

function adBreakDoneData()
{
    myGameInstance.SendMessage('Y8AdHandler', 'ResumeGame');
}

function noInterstialAdsAvailable(){
    console.log("noInterstialAdsAvailable")
    myGameInstance.SendMessage('Y8AdHandler', 'NoInterstialAdsTryLater');
}

function interstialAdsViewed(){ 
    console.log("InterstialAdsViewed")
    myGameInstance.SendMessage('Y8AdHandler', 'InterstialAdsViewed');
}

function interstialAdsStart(){ 
    console.log("InterstialAdsStart")
    myGameInstance.SendMessage('Y8AdHandler', 'InterstialAdsStart');
}
